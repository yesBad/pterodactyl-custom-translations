...
export default ({ server, className }: { server: Server; className?: string }) => {
    ...
    const diskLimit = server.limits.disk !== 0 ? megabytesToHuman(server.limits.disk) : lang.unlimited;
    const memoryLimit = server.limits.memory !== 0 ? megabytesToHuman(server.limits.memory) : lang.unlimited;
    const cpuLimit = server.limits.cpu !== 0 ? server.limits.cpu + ' %' : lang.unlimited;
                            ...
                            <div css={tw`flex-1 text-center`}>
                                <span css={tw`bg-neutral-500 rounded px-2 py-1 text-neutral-100 text-xs`}>
                                    {server.isTransferring ?
                                        'Transferring'
                                        :
                                        server.status === 'installing' ? lang.installing : (
