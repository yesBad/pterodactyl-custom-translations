...
const ServerDetailsBlock = () => {
    ...
    const statusfix = ServerContext.useStoreState(state => {
        const statsit = state.status.value;
        switch (statsit) {
            case 'offline':
                return lang.offline;
            case 'running':
    ...
    const diskLimit = limits.disk ? megabytesToHuman(limits.disk) : lang.unlimited;
    const memoryLimit = limits.memory ? megabytesToHuman(limits.memory) : lang.unlimited;
    const cpuLimit = limits.cpu ? limits.cpu + '%' : lang.unlimited;
    ...
    return (
        <TitledGreyBox css={tw`break-words`} title={name} icon={faServer}>
            <p css={tw`text-xs uppercase`}>
                ...
                />
                &nbsp;{!status ? lang.connecting : (isInstalling ? lang.installing : (isTransferring) ? lang.transferring : statusfix)}
                {stats.uptime > 0 &&
